...
std::string GetTexturePathFromModelAndTexPath(const std::string& modelPath, const char* texPath) {
	...
	std::cout << "pathIndex1 : " << pathIndex1 << "pathIndex2 : " << pathIndex2 << std::endl;
	std::cout << "modelPath :" << modelPath << " result: " << folderPath + texPath << std::endl;
	return folderPath + texPath;
}
...
LoadTextureFromFile(std::string& texPath) {
	...
	auto result = loadLambdaTable[ext](wtexpath,
		...
		scratchImg);
	if (FAILED(result)) {
		cout << "テクスチャ" << texPath << "の読み込みに失敗しました" << endl;
		return nullptr;
	}
...
ID3D12Resource* CreateBlackTexture() {
	...
	std::vector<unsigned char> data(4 * 4 * 4); // 縦 x 横 x 4ピクセル
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	//インデックス数が増えたため、バッファー作成、データ転送を書き換える
	...
	for(int i = 0; i < pmdMaterials.size(); ++i)
	{
		...
		materials[i].additional.edgeFlg = pmdMaterials[i].edgeFlg;
	...
	//3.マテリアル用ディスクリプタヒープを作成する
	//4.ディスクリプタヒープ上にビューを作成する
	//5.マテリアル用ルートパラメーター設定を追加する
	//6.シェーダーを書き換える

	//8.3.1 マテリアル用バッファーの作成
	...
	for(auto& m : materials)
	{
		*((MaterialForHlsl*)mapMaterial) = m.material; //データコピー
		mapMaterial += materialBuffSize; //次のアラインメント位置まですすめる(256の倍数）
	}
	materialBuff->Unmap(0, nullptr);

	//8.3.3 マテリアル用ディスクリプタヒープとビューの作成
	ID3D12DescriptorHeap* materialDescHeap = nullptr;
	D3D12_DESCRIPTOR_HEAP_DESC materialDescHeapDesc = {};
	materialDescHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
	materialDescHeapDesc.NodeMask = 0;
	materialDescHeapDesc.NumDescriptors = materialNum * 5; //マテリアル数 x(2->3(sphere map)->4(sphere alpha map)->5(toon map))
	...
	//8.4.5 シェーダーリソースビュー(SRV)の作成
	//専用のディスクリプタヒープを作らず、マテリアル用のディスクリプタヒープにまとめてしまう。
	//CBV | CBV | CBV ...
	//       ⇓
	//CBV SRV | CBV SRV | CBV SRV ...

	D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
	//BGRになってるのまじで意味がわからん。あとで変えて確かめる。
	...
	//定数２つ目
	descTblRange[1].NumDescriptors = 1; //ディスクリプタヒープは複数だが、一度に使うのは１つ
	...
	samplerDesc[1] = samplerDesc[0]; //変更点以外からコピー
	...
	while (true) {
		...
		auto cbvsrvIncSize = _dev->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV) * 5; //スフィアマップ分を追加 => 加算スフィアマップも追加
		for (auto& m : materials)
		{
			_cmdList->SetGraphicsRootDescriptorTable(1, materialH);
			_cmdList->DrawIndexedInstanced(m.indicesNum, 1, idxOffset, 0, 0);
			//ヒープポインターとインデックスを次に進める
			materialH.ptr += cbvsrvIncSize; //次のビューのために２倍すすめる => 3になった(SPH) => 4になった(SPA)
